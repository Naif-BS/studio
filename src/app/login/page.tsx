    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md shadow-xl">
        <CardHeader className="text-center">
          <CardTitle className="text-3xl font-bold text-primary">{t('login.title')}</CardTitle>
          <CardDescription>{t('login.description')}</CardDescription>
        </CardHeader>
        <CardContent>
          <Button variant="outline" className="w-full" onClick={handleOutlookSignIn} disabled={authLoading}>
            {authLoading && <Loader2 className={`${dir === 'rtl' ? 'ms-2' : 'me-2'} h-4 w-4 animate-spin`} />}
            <svg className={`${dir === 'rtl' ? 'ms-2' : 'me-2'} h-5 w-5`} viewBox="0 0 24 24" fill="currentColor">
