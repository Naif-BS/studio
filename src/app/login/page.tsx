...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await login(email, password); // Pass email and password to mock login
    // No need to setIsSubmitting(false) here as login navigates away
  };
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md shadow-xl">
        ...
        <CardContent>
          ...
          <Button variant="outline" className="w-full" onClick={handleOutlookSignIn} disabled={loading}>
            {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
